RSpec.describe CoinbaseCommerceClient::APIResources::Checkout do
  before :all do
  ...
  it "checkout should save nothing if nothing changes" do
    stub_request(:post, "#{@api_base}#{CoinbaseCommerceClient::APIResources::Checkout::RESOURCE_PATH}")
      .to_return(body: {data: {id: "id_value", key: "key_value"}}.to_json)
    checkout = @client.checkout.create(id: "id_value", key: "key_value")

    stub_request(:put, "#{@api_base}#{CoinbaseCommerceClient::APIResources::Checkout::RESOURCE_PATH}/id_value")
      .with(body: {})
  ...
  it "making a GET request with parameters should have a query string and no body" do
    stub_request(:get, "#{@api_base}#{CoinbaseCommerceClient::APIResources::Checkout::RESOURCE_PATH}")
