...
RSpec.describe CoinbaseCommerceClient::Webhook do
  ...
  it "should create Event object when valid payload and signatures" do
    event = CoinbaseCommerceClient::Webhook.construct_event(PAYLOAD_STR, SIG_HEADER, SECRET)
    expect(event).is_a? CoinbaseCommerceClient::APIResources::Event
  end

  it "should raise ParserError when invalid payload" do
    payload = "this is not valid JSON"
    expect {CoinbaseCommerceClient::Webhook.construct_event(payload, SIG_HEADER, SECRET)}
      .to raise_error JSON::ParserError
  end

  it "should raise a SignatureVerificationError from a valid JSON payload and an invalid signature header" do
    header = "bad_header"
    expect {CoinbaseCommerceClient::Webhook.construct_event(PAYLOAD_STR, header, SECRET)}
      .to raise_error CoinbaseCommerceClient::Errors::SignatureVerificationError
  end

  it "should raise invalid payload error on nil params" do
    secret = nil
    expect {CoinbaseCommerceClient::Webhook.construct_event(PAYLOAD_STR, SIG_HEADER, secret)}
      .to raise_error CoinbaseCommerceClient::Errors::WebhookInvalidPayload
  end
